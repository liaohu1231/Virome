args=commandArgs(T)
print("parament 1 is pNpS file from output of instrain，
parament 2 is a classified files for geographic distribution of viral populations,
parament 3 is name of contigs carried on these genes")
...
all_pi$classified=classified[as.character(all_pi$contigs),1]
...
for (j in 1:1000){
if(nrow(a)>10){
a_1[j,]=mean(a[sample(1:nrow(a),size=10,replace=F),1])
}else{
a_1[j,]=mean(a[,1])
}
#print(a_1)
if (nrow(c)>20 ){
c_1[j,]=mean(c[sample(1:nrow(c),size=20,replace=F),1])
}else{
c_1[j,]=mean(c[,1])
}
if(nrow(b)>30){
b_1[j,]=mean(b[sample(1:nrow(b),size=30,replace=F),1])
}else{
b_1[j,1]=mean(b[,1])
}
}
...
ggsave(filename="pNpS_populations_zone.pdf",p,width=7,height=6,device='pdf')
